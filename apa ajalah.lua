-- ZONE XD V10 FULL WORKING - ESP BARANG + CHEAT REAL (BUKAN CASE DOANG)
-- Copyright Apis - Kotak Hijau Item + Godmode Real + NoClip + Inf Stamina
        ...
        if Config.ESP_ENABLED then
            for _, obj in pairs(workspace:GetDescendants()) do
                ...
                if (obj:IsA("Model") or obj:IsA("Part") or obj:IsA("MeshPart")) and (
                    ...
                    name:find("object") or name:find("prop") or name:find("pickup") or name:find("bed") or name:find("table")
                ) then
                    AddESP(obj)
                end
            end
...
Rayfield:Notify({
    Title = "ZONE XD V10 LOADED",
    Content = "Toggle Enable ESP Barang ON → kotak hijau muncul! Godmode REAL, bukan visual. Copyright Apis",
    Duration = 8
})

print("ZONE XD V10 LOADED - CHEAT REAL JALAN - Copyright Apis")