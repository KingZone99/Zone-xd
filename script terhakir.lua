...
local function ScanLemari()
    SafeSpots={}
    for _,v in pairs(Workspace:GetDescendants())do
        if v:IsA("Part")then
            local n=v.Name:lower()
            if n:find("lemari")or n:find("almari")or n:find("kulkas")or n:find("cupboard")or n:find("cabinet")then
                table.insert(SafeSpots,v.Position+Vector3.new(0,3,0))
            end
        end
    end
    if #SafeSpots==0 then table.insert(SafeSpots,Vector3.new(0,10,0))end
end
...
local function IsItem(n)
    return n:find("kacamata")or n:find("kaca mata")or n:find("glasses")or
           ...
           n:find("kartu")or n:find("id")or n:find("ktp")or n:find("card")or
           n:find("papan")or n:find("klip")or n:find("clipboard")or
           n:find("rekam")or n:find("medis")or n:find("rekam medis")
end

local function IsPocong(n)
    return n:find("pocong")or n:find("hantu")or n:find("ghost")or n:find("setan")
end
    ...
    for _,v in pairs(Workspace:GetDescendants())do
        if v:IsA("Model")and v:FindFirstChild("HumanoidRootPart")and IsPocong(v.Name:lower())then
            ...
            if dist<Settings.Radius then
                ScanLemari()
                local target=SafeSpots[1]
                if #SafeSpots>0 then
                    local nd=9999
                    for _,p in pairs(SafeSpots)do
                        local d=(p-root.Position).Magnitude
                        if d<nd then nd, target = d, p end
                    end
                end
                IsTeleporting=true
                root.CFrame=CFrame.new(target)
                Notif("TELEPORT KE LEMARI!")
                task.wait(1)
                IsTeleporting=false
                break
            end
...
CreateMenu()
ScanLemari()
Notif("ZONE XD V9 LOADED")

print("ZONE XD V9 - COPYRIGHT: APIS (Aizen)")